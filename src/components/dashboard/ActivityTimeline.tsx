...
export function ActivityTimeline() {
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 h-fit">
        ...
        {activities.map((activity, index) => (
          <div key={activity.id} className="flex items-start gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">{activity.target}</p>
              {activity.campaign && (
                <p className="text-xs text-blue-600 dark:text-blue-400 mb-1">📋 {activity.campaign}</p>
              )}
      ...
      <button className="w-full mt-6 text-center text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium text-sm transition-colors">
        View all activity
      </button>
    </div>
  );
